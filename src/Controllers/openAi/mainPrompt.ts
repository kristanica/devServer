...
export const mainPrompt = async (req: Request, res: Response) => {
  const { inscructions, description } = req.body;
...
Prompt Message:
    Evaluate the code based on INSTRUCTIONS.  If any part of INSTRUCTIONS is not met, the code is wrong. Do not recommend to expand the code for testing purposes.  If INSTRUCTIONS is requiring a comment and there are no comments on the submitted code, the code is wrong. If INSTRUCTIONS is not requiring comments, then the code is fine without comments. Do not show how to code it, just state what is missing and suggestions. Make sure to point out if there are missing tags, example, if there is an opening tag but no closing tag.
